...
export const ProductForm = ({ product, categories }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        const { ok, product: productCreatedUpdated } = await createUpdateProduct(formData)
        if (!ok) {
            alert('Producto no se pudo actualizar')
            return
        }

        router.replace(`/admin/product/${productCreatedUpdated?.slug}`)
    }
